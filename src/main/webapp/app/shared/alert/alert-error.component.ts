...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(
    ...
    this.errorListener = eventManager.subscribe('watsonApp.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertError>).content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = eventManager.subscribe('watsonApp.httpError', (response: EventWithContent<unknown> | string) => {
        ...
        case 400: {
          ...
          if (errorHeader) {
            ...
            for (const fieldError of fieldErrors) {
              ...
              const fieldName: string = translateService.instant(`watsonApp.${fieldError.objectName as string}.${convertedField}`);
